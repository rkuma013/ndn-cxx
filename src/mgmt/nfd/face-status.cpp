...
FaceStatus::wireEncode(EncodingImpl<TAG>& encoder) const
{
  ...
  if (m_nFragmentationErrors) {
    totalLength += prependNonNegativeIntegerBlock(encoder, tlv::nfd::NFragmentationErrors,
                                                  *m_nFragmentationErrors);
  }
  if (m_nOutOverMtu) {
  ...
  if (m_nReassemblyTimeouts) {
    totalLength += prependNonNegativeIntegerBlock(encoder, tlv::nfd::NReassemblyTimeouts,
                                                  m_nReassemblyTimeouts);
  }
  if (m_nInNetInvalid) {
    totalLength += prependNonNegativeIntegerBlock(encoder, tlv::nfd::NInNetInvalid,
                                                  m_nInNetInvalid);
  }
  if (m_nAcknowledged) {
    totalLength += prependNonNegativeIntegerBlock(encoder, tlv::nfd::NAcknowledged,
                                                  m_nAcknowledged);
  }
  if (m_nRetransmitted) {
    totalLength += prependNonNegativeIntegerBlock(encoder, tlv::nfd::NRetransmitted,
                                                  m_nRetransmitted);
  }
  if (m_nRetxExhausted) {
    totalLength += prependNonNegativeIntegerBlock(encoder, tlv::nfd::NRetxExhausted,
                                                  m_nRetxExhausted);
  }
  if (m_nCongestionMarked) {
    totalLength += prependNonNegativeIntegerBlock(encoder, tlv::nfd::NCongestionMarked,
                                                  m_nCongestionMarked);
  }
  if (m_mtu) {
...
operator==(const FaceStatus& a, const FaceStatus& b)
{
  return a.getFaceId() == b.getFaceId() &&
      ...
      // need to change to optional comparison
